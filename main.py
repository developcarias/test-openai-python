...
def process_evaluation(evaluation):
    ...
    content_score_line = next((line for line in lines if line.startswith('Content Score:')), None)
    ...
    content_explanation_start = lines.index(content_score_line) + 1 if content_score_line else 0
    ...
    content_explanation = ' '.join(content_explanation_lines).strip().replace('Content Explanation:', '').strip() if content_explanation_lines else ''
    
    # Extract format score
    format_score_line = next((line for line in lines if line.startswith('Format Score:')), None)
    ...
    format_explanation_start = lines.index(format_score_line) + 1 if format_score_line else 0
    format_explanation_lines = []
    for line in lines[format_explanation_start:]:
        format_explanation_lines.append(line)
    format_explanation = ' '.join(format_explanation_lines).strip().replace('Format Explanation:', '').strip() if format_explanation_lines else ''
...
def main():
    ...
    for index, row in tickets_df.iterrows():
        ...
        if evaluation is None:
            print(f"Warning: Evaluation failed for row {index}. Skipping this row.")
            continue
        try:
            ...
            print(f"Unexpected error processing evaluation at row {index}: {e}")
            continue
